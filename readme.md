...
`quick-tap-ms` timeout from the `require-prior-idle-ms` timeout. Without the PR,
one can replace `require-prior-idle-ms = <150>` with `global-quick-tap` for a
...
require-prior-idle-ms PR and the hold-trigger-on-release PR (along with a few
